export const moviesData = [
  {
    id: Math.random().toString(),
    image: "https://image.tmdb.org/t/p/w500/db32LaOibwEliAmSL2jjDF6oDdj.jpg",
    rating: 5,
    name: "Star Wars: The Rise Of Skywalker",
    date: "December 2019",
    description:
      "The surviving members of the resistance face the First Order once again, and the legendary conflict between the Jedi and the Sith reaches its peak bringing the Skywalker saga to its end.",
  },
  {
    id: Math.random().toString(),
    image: "https://image.tmdb.org/t/p/w500/7WsyChQLEftFiDOVTGkv3hFpyyt.jpg",
    rating: 4,
    name: "Avengers: Infinity War",
    date: "April 2018",
    description:
      " The Avengers and their allies must be willing to sacrifice all in an attempt to defeat the powerful Thanos before his blitz of devastation and ruin puts an end to the universe",
  },
  {
    id: Math.random().toString(),
    image: "https://image.tmdb.org/t/p/w500/pjeMs3yqRmFL3giJy4PMXWZTTPa.jpg",
    rating: 4,
    name: "Frozen II",
    date: "November 2019",
    description:
      "Anna, Elsa, Kristoff, Olaf and Sven leave Arendelle to travel to an ancient, autumn-bound forest of an enchanted land. They set out to find the origin of Elsas powers in order to save their kingdom",
  },
  {
    id: Math.random().toString(),
    image: "https://image.tmdb.org/t/p/w500/1M91Bt3oGspda75H9eLqYZkJzgO.jpg",
    rating: 3,
    name: "Fantastic Beasts and Where to Find Them",
    date: "November 2016",
    description:
      "The adventures of writer Newt Scamander in New York's secret community of witches and wizards seventy years before Harry Potter reads his book in school",
  },
  {
    id: Math.random().toString(),
    image:
      "https://vignette.wikia.nocookie.net/themoviesgamelionhead/images/d/db/Cat_in_the_hat.jpg/revision/latest?cb=20130905001657",
    rating: 2,
    name: "Cat In The Hat",
    date: "November 2003",
    description:
      "Wow, we REALLY got angry on this one. Here are Doug and Robs thoughts on the 2003 Cat in the Hat",
  },
];
